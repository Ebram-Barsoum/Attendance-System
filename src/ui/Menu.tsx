...
function List({
    ...
    return (
        <div
            className={`w-[10rem] flex flex-col rounded-lg bg-white overflow-hidden absolute z-10 right-[100%] top-[100%] ${className}`}
            ref={ref}
        >
            {children}
        </div>
...
function Option({
    ...
    return (
        <div
            onClick={handleClick}
            className="p-2 w-full flex items-center gap-2 hover:bg-stone-100 text-gray-800 border-b cursor-pointer last:border-0"
        >
            {children}
        </div>
    );
